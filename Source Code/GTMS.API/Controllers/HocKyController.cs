...
namespace GTMS.API.Controllers
{
    ...
    {
        private readonly IHocKyRepository _repo;
        private readonly IMapper _mapper;
        private readonly string _entityName;
        public HocKyController(IHocKyRepository repo, IMapper mapper)
        {
            _repo = repo;
            _mapper = mapper;
            _entityName = "học kỳ";
        ...
        //[Authorize(Roles = "NQL")]
        [HttpGet]
        public async Task<IActionResult> GetAll([FromQuery] HocKyParams userParams)
        {

            try
            {
                var result = await _repo.GetAll(userParams);
                var resultToReturn = _mapper.Map<IEnumerable<HocKyForListDto>>(result);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var resultToReturn = _mapper.Map<HocKyForViewDto>(result);
        ...
        //[Authorize(Roles = "NQL")]
        [HttpPost]
        public async Task<IActionResult> Create(HocKyForCreateDto hocKy)
        {
            try
            {
                var validationResult = _repo.ValidateBeforeCreate(hocKy);

                if (validationResult.IsValid)
                {
                    var result = await _repo.Create(hocKy);
        ...
        [HttpPost]
        public async Task<IActionResult> CreateMultiple(ICollection<HocKyForCreateMultipleDto> danhSachHocKy)
        {
            try
            {
                var validationResult = _repo.ValidateBeforeCreateMultiple(danhSachHocKy);

                if (validationResult.IsValid)
                {
                    var result = await _repo.CreateMultiple(danhSachHocKy);
