...
namespace GTMS.API.Controllers
{
    ...
    {
        private readonly ILopRepository _repo;
 ...
 //       [Authorize(Roles = "NQL")]
        [HttpGet]
        public async Task<IActionResult> GetAll([FromQuery] LopParams userParams)
        {
            try
            {
                var result = await _repo.GetAll(userParams);
                var resultToReturn = _mapper.Map<IEnumerable<LopForListDto>>(result);
...
//        [Authorize(Roles = "NQL")]
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var resultToReturn = _mapper.Map<LopForViewDto>(result);
...
//        [Authorize(Roles = "NQL")]
        ...
        [HttpPost]
        public async Task<IActionResult> CreateMultiple(ICollection<LopForCreateMultipleDto> danhSachLop)
        {
            try
            {
                var validationResult = _repo.ValidateBeforeCreateMultiple(danhSachLop);

                if (validationResult.IsValid)
                {
                    var result = await _repo.CreateMultiple(danhSachLop);
        ...
        //        [Authorize(Roles = "NQL")]
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateById(int id, LopForUpdateDto lop)
        {
            try
            {
                var validationResult = _repo.ValidateBeforeUpdate(id, lop);

                if (validationResult.IsValid)
                {
                    var result = await _repo.UpdateById(id, lop);
