...
class QuyDinhForCreate extends Component {
    constructor (props) {
      super(props)
      this.state = {
          editingData: {
              tenQuyDinh: '',
              thoiGianBatDauHieuLuc: moment().format('YYYY-MM-DD'),
              soSVTHToiThieu: 0,
              soSVTHToiDa: 0,
              soGVHDToiThieu: 0,
              soGVHDToiDa: 0,
    ...
    submit() {
        ...
        if (tenQuyDinh !== '' && thoiGianBatDauHieuLuc !== '') {
            const url = apiRoutes.quyDinh.create
        ...
        return (
            <Fragment>
                ...
                <span className='breadcrumb-home'><Link to='/'>Graduation Thesis Management System (GTMS)</Link></span>
                    ...
                    <span className='breadcrumb-separator'><i className='fas fa-chevron-right'></i></span>
                    <span className='breadcrumb-active'><Link to='#'>Thêm quy định mới</Link></span>
                </section>

                <section className='section'>
                    <header className='section__header'>
                        <div className='section__header-left'>
                            <h3 className='section__title'>Thêm quy định mới</h3>
                            <p className='section__subtitle'>Thêm quy định mới vào hệ thống</p>
                        </div>
                    </header>
                        ...
                        <div className='section-body-main'>
                            <form autoComplete='off'>
                                ...
                                <div className='form-group'>
                                    ...
                                    <input className={ this.isValidInputField('thoiGianBatDauHieuLuc') ? 'form-input-alert' : 'form-input-outline' } type='date' placeholder='Nhập thời gian bắt đầu hiệu lực của quy định' value={this.state.editingData.thoiGianBatDauHieuLuc} onChange={e => this.changeEditingData(e, 'thoiGianBatDauHieuLuc')} onFocus={ () => this.hideAlert() } />
                                </div>
...
export default connect(mapStateToProps, null)(QuyDinhForCreate)