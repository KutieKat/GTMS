...
class KhoaForList extends Component {
    constructor (props) {
      ...
      this.fileReader.onload = (e) => {
        try {
            ...
            const fieldToParseNumber = ['maQuyDinh', 'trangThai']
            let data
    ...
    async createMultiple() {
        const url = apiRoutes.quyDinh.createMultiple
    ...
    async fetchData() {
        const url = apiRoutes.quyDinh.getAll
    ...
    async onTemporarilyDelete(id) {
        try {
            const url = apiRoutes.quyDinh.temporarilyDeleteById + '/' + id
    ...
    async onPermanentlyDelete(id) {
        try {
            const url = apiRoutes.quyDinh.permanentlyDeleteById + '/' + id
    ...
    async onRestore(id) {
        try {
            const url = apiRoutes.quyDinh.restoreById + '/' + id
            const data = { trangThai: 1 }
    ...
    changeConfirmationValue(e) {
        ...
        const acceptedValue = this.state.activeRecord.maQuyDinh.toString()
    ...
    exportDocumentAsXlsx() {
        ...
        worksheet.getCell('A4').value = 'DANH SÁCH CÁC QUY ĐỊNH HIỆN CÓ TRONG HỆ THỐNG'
        ...
        worksheet.getCell('C7').value = 'Tên quy định'
        ...
        worksheet.getCell('J7').value = 'Tên viết tắt của quy định'
        ...
        return (
            <Fragment>
                <section className='breadcrumbs'>
                    ...
                    <span className='breadcrumb-active'><Link to='/quy-dinh'>Quản lý quy định</Link></span>
                </section>

                <section className='section'>
                    <header className='section__header'>
                        <div className='section__header-left'>
                            <h3 className='section__title'>Danh sách quy định</h3>
                            <p className='section__subtitle'>Danh sách tất cả các quy định hiện đang được quản lý</p>
                        </div>
                        ...
                        <div className='section-body-main'>
                            ...
                            <table className='table'>
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th onClick={ () => this.sortByColumn('TenQuyDinh') }>{ this.isBeingSorted('TenQuyDinh') && <i className='fas fa-sort sort-button'></i> } Tên quy định</th>
                                        <th onClick={ () => this.sortByColumn('ThoiGianBatDauHieuLuc') }>{ this.isBeingSorted('ThoiGianBatDauHieuLuc') && <i className='fas fa-sort sort-button'></i> } Thời gian bắt đầu hiệu lực</th>
                                            ...
                                            <tr key={ index }>
                                                <td className='table-dropdown-menu-wrapper'>
                                                    ...
                                                    <ul className='table-dropdown-menu'>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/quy-dinh/xem-thong-tin/' + record.maQuyDinh }>Xem thông tin</Link></li>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/quy-dinh/cap-nhat/' + record.maQuyDinh }>Cập nhật thông tin</Link></li>
                                                ...
                                                <td>{ record.tenQuyDinh }</td>
                                                <td>{ formatDateString(record.thoiGianBatDauHieuLuc, false) }</td>
                ...
                { this.state.showPopup.temporarilyDelete && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn xóa tạm thời quy định <strong>{ this.state.activeRecord.tenKhoa }</strong> khỏi hệ thống hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onTemporarilyDelete(this.state.activeRecord.maQuyDinh) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                    ...
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <Fragment>
                                <p className='alert-message'>Bạn có chắc chắn muốn xóa vĩnh viễn quy định <strong>{ this.state.activeRecord.tenKhoa }</strong> khỏi hệ thống hay không?</p>
                                <p>Hành động này đồng nghĩa với việc dữ liệu sẽ bị xóa vĩnh viễn khỏi hệ thống và không thể khôi phục lại được. Để xác nhận, vui lòng nhập <span className='pre'>{ this.state.activeRecord.maQuyDinh }</span> vào khung bên dưới</p>
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <Fragment>
                                { this.state.validConfirmationValue ? <span className='popup-button-danger' onClick={ () => this.onPermanentlyDelete(this.state.activeRecord.maQuyDinh) }>Đồng ý</span> : <span className='popup-button-default'>Đồng ý</span> }
                            </Fragment> }
                        </footer>
                    </div>
                </div> }
                ...
                { this.state.showPopup.restore && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn khôi phục lại quy định <strong>{ this.state.activeRecord.tenKhoa }</strong> đã bị xóa tạm thời hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onRestore(this.state.activeRecord.maQuyDinh) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
