...
import LopForPrint from './LopForPrint'
...
class LopForList extends Component {
    constructor (props) {
      super(props)
      this.state = {
          data: [],
      ...
      this.fileReader.onload = (e) => {
        try {
            ...
            const fieldToParseNumber = ['maLop', 'maKhoa', 'maKhoaDaoTao', 'trangThai']
            let data
    ...
    async createMultiple() {
        const url = apiRoutes.lop.createMultiple
    ...
    async onTemporarilyDelete(id) {
        try {
            const url = apiRoutes.lop.temporarilyDeleteById + '/' + id
    ...
    async onPermanentlyDelete(id) {
        try {
            const url = apiRoutes.lop.permanentlyDeleteById + '/' + id
    ...
    async onRestore(id) {
        try {
            const url = apiRoutes.lop.restoreById + '/' + id
            const data = { trangThai: 1 }
    ...
    changeConfirmationValue(e) {
        ...
        const acceptedValue = this.state.activeRecord.maLop.toString()
            ...
            case 'CURRENT_PAGE': {
                ...
                dataToExport = dataToExport.map(record => {
                    return {
                        ...
                        maKhoa: record.khoa.maKhoa,
                        maKhoaDaoTao: record.khoaDaoTao.maKhoaDaoTao,
                        heDaoTao: record.heDaoTao,
                        thoiGianTao: record.thoiGianTao,
                        thoiGianCapNhat: record.thoiGianCapNhat,
    ...
    exportDocumentAsXlsx() {
        ...
        worksheet.getCell('A4').value = 'DANH SÁCH CÁC LỚP HIỆN CÓ TRONG HỆ THỐNG'
        ...
        worksheet.getCell('C7').value = 'Tên lớp'
        ...
        worksheet.getCell('F7').value = 'Tên viết tắt'
        worksheet.getCell('F7').font = excelFormat.boldFont
        worksheet.getCell('F7').alignment = excelFormat.center
        worksheet.getCell('F7').border = excelFormat.border

        worksheet.mergeCells('H7:J7')
        worksheet.getCell('H7').value = 'Khoa'
        worksheet.getCell('H7').font = excelFormat.boldFont
        worksheet.getCell('H7').alignment = excelFormat.center
        worksheet.getCell('H7').border = excelFormat.border

        worksheet.mergeCells('K7:M7')
        worksheet.getCell('K7').value = 'Khóa đào tạo'
        ...
        worksheet.getCell('N7').value = 'Hệ đào tạo'
        worksheet.getCell('N7').font = excelFormat.boldFont
        worksheet.getCell('N7').alignment = excelFormat.center
        worksheet.getCell('N7').border = excelFormat.border
        ...
        this.state.dataToExportDocument.forEach((record, index) => {
            ...
            worksheet.getCell('C' + currentRowIndex).value = record.tenLop
            ...
            worksheet.getCell('F' + currentRowIndex).value = record.tenVietTat
            worksheet.getCell('F' + currentRowIndex).font = excelFormat.normalFont
            worksheet.getCell('F' + currentRowIndex).alignment = excelFormat.left
            worksheet.getCell('F' + currentRowIndex).border = excelFormat.border

            worksheet.mergeCells('H' + currentRowIndex + ':J' + currentRowIndex)
            worksheet.getCell('H' + currentRowIndex).value = record.khoa.tenKhoa
            worksheet.getCell('H' + currentRowIndex).font = excelFormat.normalFont
            worksheet.getCell('H' + currentRowIndex).alignment = excelFormat.left
            worksheet.getCell('H' + currentRowIndex).border = excelFormat.border

            worksheet.mergeCells('K' + currentRowIndex + ':M' + currentRowIndex)
            worksheet.getCell('K' + currentRowIndex).value = record.khoaDaoTao.tenKhoaDaoTao
            ...
            worksheet.mergeCells('N' + currentRowIndex + ':P' + currentRowIndex)
            worksheet.getCell('N' + currentRowIndex).value = record.heDaoTao
            worksheet.getCell('N' + currentRowIndex).font = excelFormat.normalFont
            worksheet.getCell('N' + currentRowIndex).alignment = excelFormat.left
            worksheet.getCell('N' + currentRowIndex).border = excelFormat.border
        })
        ...
        return (
            <Fragment>
                <section className='breadcrumbs'>
                    ...
                    <span className='breadcrumb-active'><Link to='/lop'>Quản lý lớp</Link></span>
                </section>

                <section className='section'>
                    <header className='section__header'>
                        <div className='section__header-left'>
                            <h3 className='section__title'>Danh sách lớp</h3>
                            <p className='section__subtitle'>Danh sách tất cả các lớp hiện đang được quản lý</p>
                        </div>
                        ...
                        <div className='section-body-main'>
                            ...
                            <table className='table'>
                                <thead>
                                    <tr>
                                        <th></th>
                                        {/* <th onClick={ () => this.sortByColumn('MaLop') }>{ this.isBeingSorted('MaLop') && <i className='fas fa-sort sort-button'></i> } Mã lớp</th> */}
                                        <th onClick={ () => this.sortByColumn('TenLop') }>{ this.isBeingSorted('TenLop') && <i className='fas fa-sort sort-button'></i> } Tên lớp</th>
                                            ...
                                            <tr key={ index }>
                                                <td className='table-dropdown-menu-wrapper'>
                                                    ...
                                                    <ul className='table-dropdown-menu'>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/lop/xem-thong-tin/' + record.maLop }>Xem thông tin</Link></li>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/lop/cap-nhat/' + record.maLop }>Cập nhật thông tin</Link></li>
                                                ...
                                                {/* <td className='table-text-bold'>{ record.maLop }</td> */}
                                                <td>{ record.tenLop }</td>
                                                <td>{ record.tenVietTat }</td>
                                                <td>{ record.khoa.tenKhoa }</td>
                                                <td>{ record.khoaDaoTao.tenKhoaDaoTao }</td>
                                                <td>{ record.heDaoTao }</td>
                ...
                { this.state.showPopup.temporarilyDelete && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn xóa tạm thời lớp <strong>{ this.state.activeRecord.tenLop }</strong> khỏi hệ thống hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onTemporarilyDelete(this.state.activeRecord.maLop) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                    ...
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <Fragment>
                                <p className='alert-message'>Bạn có chắc chắn muốn xóa vĩnh viễn lớp <strong>{ this.state.activeRecord.tenLop }</strong> khỏi hệ thống hay không?</p>
                                <p>Hành động này đồng nghĩa với việc dữ liệu sẽ bị xóa vĩnh viễn khỏi hệ thống và không thể khôi phục lại được. Để xác nhận, vui lòng nhập <span className='pre'>{ this.state.activeRecord.maLop }</span> vào khung bên dưới</p>
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <Fragment>
                                { this.state.validConfirmationValue ? <span className='popup-button-danger' onClick={ () => this.onPermanentlyDelete(this.state.activeRecord.maLop) }>Đồng ý</span> : <span className='popup-button-default'>Đồng ý</span> }
                            </Fragment> }
                        </footer>
                    </div>
                </div> }
                ...
                { this.state.showPopup.restore && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn khôi phục lại lớp <strong>{ this.state.activeRecord.tenLop }</strong> đã bị xóa tạm thời hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onRestore(this.state.activeRecord.maLop) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                ...
                <LopForPrint data={ this.state.dataToExportDocument } />
            </Fragment>
        )
    }
}
...
export default connect(mapStateToProps, null)(LopForList)