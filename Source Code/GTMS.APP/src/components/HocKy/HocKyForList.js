...
import HocKyForPrint from './HocKyForPrint'
...
class HocKyForList extends Component {
    constructor (props) {
      super(props)
      this.state = {
          data: [],
      ...
      this.fileReader.onload = (e) => {
        try {
            ...
            const fieldToParseNumber = ['maHocKy', 'trangThai']
            let data
    ...
    async createMultiple() {
        const url = apiRoutes.hocKy.createMultiple
    ...
    async onTemporarilyDelete(id) {
        try {
            const url = apiRoutes.hocKy.temporarilyDeleteById + '/' + id
    ...
    async onPermanentlyDelete(id) {
        try {
            const url = apiRoutes.hocKy.permanentlyDeleteById + '/' + id
    ...
    async onRestore(id) {
        try {
            const url = apiRoutes.hocKy.restoreById + '/' + id
            const data = { trangThai: 1 }
    ...
    changeConfirmationValue(e) {
        ...
        const acceptedValue = this.state.activeRecord.maHocKy.toString()
    ...
    exportDocumentAsXlsx() {
        ...
        worksheet.getCell('C7').value = 'Tên học kỳ'
        ...
        this.state.dataToExportDocument.forEach((record, index) => {
            ...
            worksheet.getCell('C' + currentRowIndex).value = record.tenHocKy
        ...
        return (
            <Fragment>
                <section className='breadcrumbs'>
                    ...
                    <span className='breadcrumb-active'><Link to='hoc-ky'>Quản lý học kỳ</Link></span>
                </section>

                <section className='section'>
                    <header className='section__header'>
                        <div className='section__header-left'>
                            <h3 className='section__title'>Danh sách học kỳ</h3>
                            <p className='section__subtitle'>Danh sách tất cả các học kỳ hiện đang được quản lý</p>
                        </div>
                        ...
                        <div className='section-body-main'>
                            ...
                            <table className='table'>
                                <thead>
                                    <tr>
                                        <th></th>
                                        {/* <th onClick={ () => this.sortByColumn('MaHocKy') }>{ this.isBeingSorted('MaHocKy') && <i className='fas fa-sort sort-button'></i> } Mã học kỳ</th> */}
                                        <th onClick={ () => this.sortByColumn('TenHocKy') }>{ this.isBeingSorted('TenHocKy') && <i className='fas fa-sort sort-button'></i> } Tên học kỳ</th>
                                        <th onClick={ () => this.sortByColumn('NamHoc') }>{ this.isBeingSorted('NamHoc') && <i className='fas fa-sort sort-button'></i> } Năm học</th>
                                            ...
                                            <tr key={ index }>
                                                <td className='table-dropdown-menu-wrapper'>
                                                    ...
                                                    <ul className='table-dropdown-menu'>
                                                        <li className='table-dropdown-menu-item'><Link to={ 'hoc-ky/xem-thong-tin/' + record.maHocKy }>Xem thông tin</Link></li>
                                                        <li className='table-dropdown-menu-item'><Link to={ 'hoc-ky/cap-nhat/' + record.maHocKy }>Cập nhật thông tin</Link></li>
                                                ...
                                                {/* <td className='table-text-bold'>{ record.maHocKy }</td> */}
                                                <td>{ record.tenHocKy }</td>
                                                <td>{ record.namHoc }</td>
                ...
                { this.state.showPopup.temporarilyDelete && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn xóa tạm thời học kỳ <strong>{ this.state.activeRecord.tenHocKy }</strong> khỏi hệ thống hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onTemporarilyDelete(this.state.activeRecord.maHocKy) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                    ...
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <Fragment>
                                <p className='alert-message'>Bạn có chắc chắn muốn xóa vĩnh viễn học kỳ <strong>{ this.state.activeRecord.tenHocKy }</strong> khỏi hệ thống hay không?</p>
                                <p>Hành động này đồng nghĩa với việc dữ liệu sẽ bị xóa vĩnh viễn khỏi hệ thống và không thể khôi phục lại được. Để xác nhận, vui lòng nhập <span className='pre'>{ this.state.activeRecord.maHocKy }</span> vào khung bên dưới</p>
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <Fragment>
                                { this.state.validConfirmationValue ? <span className='popup-button-danger' onClick={ () => this.onPermanentlyDelete(this.state.activeRecord.maHocKy) }>Đồng ý</span> : <span className='popup-button-default'>Đồng ý</span> }
                            </Fragment> }
                        </footer>
                    </div>
                </div> }
                ...
                { this.state.showPopup.restore && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn khôi phục lại học kỳ <strong>{ this.state.activeRecord.tenHocKy }</strong> đã bị xóa tạm thời hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onRestore(this.state.activeRecord.maHocKy) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                ...
                <HocKyForPrint data={ this.state.dataToExportDocument } />
            </Fragment>
        )
    }
}
...
export default connect(mapStateToProps, null)(HocKyForList)