...
import DoAnForPrint from './DoAnForPrint'
...
class DoAnForList extends Component {
    constructor (props) {
      super(props)
      this.state = {
          data: [],
      ...
      this.fileReader.onload = (e) => {
        try {
            ...
            const fieldToParseNumber = ['maDoAn', 'trangThai']
            let data
    ...
    async createMultiple() {
        const url = apiRoutes.doAn.createMultiple
    ...
    async fetchData() {
        const url = apiRoutes.doAn.getAll
    ...
    async fetchHuongNghienCuu() {
        ...
        try {
            ...
            if (response && response.data.status === 'SUCCESS') {
                ...
                this.setState({ huongNghienCuu: data })
            }
            else {
                throw new Error(response.errors)
            }
    ...
    async fetchHuongDanDoAn() {
        const url = apiRoutes.huongDanDoAn.getAll
        const params = {}
        params['pageSize'] = 1000
        params['trangThai'] = 1
        params['keyword'] = this.state.gvhdKeyword
    ...
    async onTemporarilyDelete(id) {
        try {
            const url = apiRoutes.doAn.temporarilyDeleteById + '/' + id
    ...
    async onPermanentlyDelete(id) {
        try {
            const url = apiRoutes.doAn.permanentlyDeleteById + '/' + id
    ...
    async onRestore(id) {
        try {
            const url = apiRoutes.doAn.restoreById + '/' + id
            const data = { trangThai: 1 }
    ...
    changeConfirmationValue(e) {
        ...
        const acceptedValue = this.state.activeRecord.maDoAn.toString()
    ...
    exportDocumentAsXlsx() {
        ...
        worksheet.getCell('A4').value = 'DANH SÁCH CÁC HỌC KỲ HIỆN CÓ TRONG HỆ THỐNG'
        ...
        worksheet.getCell('C7').value = 'Tên đồ án'
        ...
        worksheet.getCell('G7').value = 'Năm học'
        ...
        worksheet.getCell('I7').value = 'Thời gian bắt đầu'
        ...
        worksheet.getCell('M7').value = 'Thời gian kết thúc'
        ...
        this.state.dataToExportDocument.forEach((record, index) => {
            ...
            worksheet.getCell('C' + currentRowIndex).value = record.tenDoAn
            ...
            worksheet.mergeCells('G' + currentRowIndex + ':H' + currentRowIndex)
            worksheet.getCell('G' + currentRowIndex).value = record.namHoc
            ...
            worksheet.mergeCells('I' + currentRowIndex + ':L' + currentRowIndex)
            worksheet.getCell('I' + currentRowIndex).value = formatDateString(record.thoiGianBatDau, false)
            ...
            worksheet.mergeCells('M' + currentRowIndex + ':P' + currentRowIndex)
            worksheet.getCell('M' + currentRowIndex).value = formatDateString(record.thoiGianKetThuc, false)
        ...
        return (
            <Fragment>
                <section className='breadcrumbs'>
                    ...
                    <span className='breadcrumb-active'><Link to='/do-an'>Quản lý đồ án</Link></span>
                </section>

                <section className='section'>
                    <header className='section__header'>
                        <div className='section__header-left'>
                            <h3 className='section__title'>Danh sách đồ án</h3>
                            <p className='section__subtitle'>Danh sách tất cả các đồ án hiện đang được quản lý</p>
                        </div>
                    ...
                    <div className='section__body'>
                        <div className='section-body-main'>
                            {/* <div>
                                <select className='form-input-outline'>
                                    {
                                        this.state.huongNghienCuu.map(record => {
                                            return <option valud={ record.maHuongNghienCuu }>{ record.tenHuongNghienCuu }</option>
                                        })
                                    }
                                </select>
                            </div> */}
                            ...
                            <table className='table'>
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th onClick={ () => this.sortByColumn('TenDoAn') }>{ this.isBeingSorted('TenDoAn') && <i className='fas fa-sort sort-button'></i> } Tên đồ án</th>
                                        <th onClick={ () => this.sortByColumn('MaHuongNghienCuu') }>{ this.isBeingSorted('MaHuongNghienCuu') && <i className='fas fa-sort sort-button'></i> } Hướng nghiên cứu</th>
                                        <th onClick={ () => this.sortByColumn('NhomSinhVienThucHien') }>{ this.isBeingSorted('NhomSinhVienThucHien') && <i className='fas fa-sort sort-button'></i> } Nhóm sinh viên thực hiện</th>
                                        <th onClick={ () => this.sortByColumn('ThoiGianBaoCao') }>{ this.isBeingSorted('ThoiGianBaoCao') && <i className='fas fa-sort sort-button'></i> } Thời gian báo cáo</th>
                                        <th onClick={ () => this.sortByColumn('DiemTongKet') }>{ this.isBeingSorted('DiemTongKet') && <i className='fas fa-sort sort-button'></i> } Điểm tổng kết</th>
                                        <th onClick={ () => this.sortByColumn('TaiDoAn') }>{ this.isBeingSorted('TaiDoAn') && <i className='fas fa-sort sort-button'></i> } Tải đồ án</th>
                                            ...
                                            <tr key={ index }>
                                                <td className='table-dropdown-menu-wrapper'>
                                                    ...
                                                    <ul className='table-dropdown-menu'>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/do-an/xem-thong-tin/' + record.maDoAn }>Xem thông tin</Link></li>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/do-an/cap-nhat/' + record.maDoAn }>Cập nhật thông tin</Link></li>
                                                ...
                                                <td>{ record.diemTongKet }</td>
                                                <td>
                                                    <a href={ record.lienKetTaiDoAn } target='_blank'>Tải về</a>
                                                </td>
                ...
                { this.state.showPopup.temporarilyDelete && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn xóa tạm thời đồ án <strong>{ this.state.activeRecord.tenDoAn }</strong> khỏi hệ thống hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onTemporarilyDelete(this.state.activeRecord.maDoAn) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                    ...
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <Fragment>
                                <p className='alert-message'>Bạn có chắc chắn muốn xóa vĩnh viễn đồ án <strong>{ this.state.activeRecord.tenDoAn }</strong> khỏi hệ thống hay không?</p>
                                <p>Hành động này đồng nghĩa với việc dữ liệu sẽ bị xóa vĩnh viễn khỏi hệ thống và không thể khôi phục lại được. Để xác nhận, vui lòng nhập <span className='pre'>{ this.state.activeRecord.maDoAn }</span> vào khung bên dưới</p>
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <Fragment>
                                { this.state.validConfirmationValue ? <span className='popup-button-danger' onClick={ () => this.onPermanentlyDelete(this.state.activeRecord.maDoAn) }>Đồng ý</span> : <span className='popup-button-default'>Đồng ý</span> }
                            </Fragment> }
                        </footer>
                    </div>
                </div> }
                ...
                { this.state.showPopup.restore && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn khôi phục lại đồ án <strong>{ this.state.activeRecord.tenDoAn }</strong> đã bị xóa tạm thời hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onRestore(this.state.activeRecord.maDoAn) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                ...
                <DoAnForPrint data={ this.state.dataToExportDocument } />
            </Fragment>
        )
    }
}
...
export default connect(mapStateToProps, null)(DoAnForList)