...
import { formatDateString, standardizeScore, getValue } from '../../utils'
...
class DoAnForCreate extends Component {
    ...
    async fetchDoAn() {
        ...
        const url = apiRoutes.doAn.getById + '/' + id
                ...
                this.setState({ 
                    ...
                    editingData: { 
                        ...
                        sinhVienThucHien: (() => {
                            ...
                            response.data.result.data.huongNghienCuu.doAn[0].sinhVien.forEach(d => {
                                let record = this.state.sinhVien.find(r => r.maSinhVien === d.maSinhVien)
                                results.push(record)
                            })

                            return results
                        })(),
                        thucHienDoAn: response.data.result.data.huongNghienCuu.doAn[0].sinhVien.map(r => r.maSinhVien),
                        giangVienHuongDan: (() => {
                            let results = []

                            response.data.result.data.huongNghienCuu.doAn[0].huongDanDoAn.forEach(d => {
                                let record = this.state.giangVien.find(r => r.maGiangVien === d.maGiangVien)
                                record['nhanXet'] = d.nhanXet
                        ...
                        huongDanDoAn: response.data.result.data.huongNghienCuu.doAn[0].huongDanDoAn.map(r => {
                            return { maGiangVien: r.maGiangVien, nhanXet: r.nhanXet }
                        }),
                        giangVienPhanBien: (() => {
                            let results = []

                            response.data.result.data.huongNghienCuu.doAn[0].phanBienDoAn.forEach(d => {
                                let record = this.state.giangVien.find(r => r.maGiangVien === d.maGiangVien)
                                record['nhanXet'] = d.nhanXet
                        ...
                        phanBienDoAn: response.data.result.data.huongNghienCuu.doAn[0].phanBienDoAn.map(r => {
                            return { maGiangVien: r.maGiangVien, nhanXet: r.nhanXet }
                        }),
                        hoiDongBaoVe: (() => {
                            let results = []

                            response.data.result.data.huongNghienCuu.doAn[0].thanhVienHDBV.forEach(d => {
                                let record = this.state.giangVien.find(r => r.maGiangVien === d.maGiangVien)
                        ...
                        baoVeDoAn: response.data.result.data.huongNghienCuu.doAn[0].thanhVienHDBV.map(r => {
                            return { maGiangVien: r.maGiangVien, chucVu: r.chucVu }
                        }),
                        diemDoAn: (() => {
                            let results = []

                            response.data.result.data.huongNghienCuu.doAn[0].diemDoAn.forEach(d => {
                                let record = this.state.giangVien.find(r => r.maGiangVien === d.maGiangVien)
                                results.push({ ...record, ...d })
                            })

                            return results
                        })()
                    }
                }, this.calculateDiemTongKet)
            } else {
        ...
        return (
            <Fragment>
                ...
                <span className='breadcrumb-home'><Link to='/'>Graduation Thesis Management System (GTMS)</Link></span>
                    ...
                    <span className='breadcrumb-separator'><i className='fas fa-chevron-right'></i></span>
                    <span className='breadcrumb-active'><Link to='#'>Xem thông tin đồ án</Link></span>
                </section>

                <section className='section'>
                    <header className='section__header'>
                        <div className='section__header-left'>
                            <h3 className='section__title'>Xem thông tin đồ án</h3>
                            <p className='section__subtitle'>Xem thông tin chi tiết về đồ án</p>
                        </div>
                    </header>
                        ...
                        <div className='section-body-main'>
                            <form autoComplete='off'>
                                ...
                                <div className='form-group'>
                                    ...
                                    <input type='text' className={ this.isValidInputField('tenDoAn') ? 'form-input-alert' : 'form-input-disabled' } placeholder='Nhập tên của đồ án' value={ this.state.data.maDoAn } disabled />
                                </div>

                                <div className='form-group'>
                                    <label className='form-label'>Tên đồ án</label>
                                    <input type='text' className={ this.isValidInputField('tenDoAn') ? 'form-input-alert' : 'form-input-disabled' } placeholder='Nhập tên của đồ án' value={ getValue(this.state.data.tenDoAn) } disabled />
                                </div>

                                <div className='form-group'>
                                    <label className='form-label'>Mô tả về đồ án</label>
                                    <textarea className={ this.isValidInputField('moTa') ? 'form-input-alert' : 'form-input-disabled' } placeholder='Nhập mô tả về đồ án' value={ getValue(this.state.data.moTa) } disabled></textarea>
                                </div>

                                <div className='form-group'>
                                    <label className='form-label'>Hướng nghiên cứu</label>
                                    <select className={this.isValidInputField('maHuongNghienCuu') ? 'form-input-alert' : 'form-input-disabled'} value={this.state.data.maHuongNghienCuu} disabled>
                                        {
                                ...
                                <div className='form-group'>
                                    ...
                                    <select className={this.isValidInputField('maHocKy') ? 'form-input-alert' : 'form-input-disabled'} value={this.state.data.maHocKy} onChange={e => this.changeEditingData(e, 'maHocKy')} onFocus={() => this.hideAlert()} disabled>
                                        {
                                ...
                                <div className='form-group'>
                                    ...
                                    <input type='date' className={ this.isValidInputField('thoiGianBaoCao') ? 'form-input-alert' : 'form-input-disabled' } placeholder='Nhập thời gian báo cáo đồ án' value={ moment(this.state.data.thoiGianBaoCao).format('YYYY-MM-DD') } disabled />
                                </div>

                                <div className='form-group'>
                                    <label className='form-label'>Địa điểm báo cáo</label>
                                    <input type='text' className={ this.isValidInputField('diaDiemBaoCao') ? 'form-input-alert' : 'form-input-disabled' } placeholder='Nhập địa điểm báo cáo đồ án' value={ getValue(this.state.data.diaDiemBaoCao) } disabled />
                                </div>

                                <div className='form-group'>
                                    <label className='form-label'>Liên kết tải đồ án</label>
                                    <input type='text' className={ this.isValidInputField('lienKetTaiDoAn') ? 'form-input-alert' : 'form-input-disabled' } placeholder='Nhập liên kết tải đồ án' value={ getValue(this.state.data.lienKetTaiDoAn) } disabled/>
                                </div>
                                    ...
                                    <tbody>
                                        {
                                            this.state.editingData.sinhVienThucHien.length > 0 ? this.state.editingData.sinhVienThucHien.map((record, index) => (
                                                <tr key={ index }>
                                                    <td>{ index + 1 }</td>
                                                    <td className='table-text-bold'>{ record && record.maSinhVien }</td>
                                                    <td>{ record && record.hoVaTen }</td>
                                                    <td>{ record && record.gioiTinh }</td>
                                                    <td>{ formatDateString(record && record.ngaySinh, false) }</td>
                                                    <td>{ record && record.lop && record.lop.tenLop }</td>
                                                    <td>{ record && record.lop && record.lop.khoaDaoTao.tenKhoaDaoTao }</td>
                                                    <td>{ record && record.lop && record.lop.heDaoTao }</td>
                                                </tr>
                                            )) :
                                            <tr>
                                    ...
                                    <tbody>
                                        {
                                            this.state.editingData.giangVienHuongDan.length > 0 ? this.state.editingData.giangVienHuongDan.map((record, index) => (
                                                <tr key={ index }>
                                                    <td>{ index + 1 }</td>
                                                    <td className='table-text-bold'>{ record && record.maGiangVien }</td>
                                                    <td>{ record && record.hoVaTen }</td>
                                                    <td>{ record && record.hocHam }</td>
                                                    <td>{ record && record.hocVi}</td>
                                                    <td>{ record && record.khoa && record.khoa.tenKhoa }</td>
                                                    <td>{ record && record.donViCongTac }</td>
                                                </tr>
                                            )) :
                                            <tr>
                                    ...
                                    <tbody>
                                        {
                                            this.state.editingData.giangVienPhanBien.length > 0 ? this.state.editingData.giangVienPhanBien.map((record, index) => (
                                                <tr key={ index }>
                                                    <td>{ index + 1 }</td>
                                                    <td className='table-text-bold'>{ record.maGiangVien }</td>
                                ...
                                <div className='form-group'>
                                    <textarea className={ this.isValidInputField('nhanXetChung') ? 'form-input-alert' : 'form-input-disabled' } placeholder='Nhập nhận xét chung của hội đồng bảo vệ đồ án' value={ this.state.data.nhanXetChung } onChange={ e => this.changeEditingData(e, 'nhanXetChung') } onFocus={ () => this.hideAlert() } disabled></textarea>
                                </div>
                    ...
                    { this.state.showPopup.selectSVTH && <div className='popup-wrapper'>
                        <div className='popup'>
                            ...
                            <div className='popup__body'>
                                <div className='form-group'>
                                    <input type='text' className='form-input-disabled' placeholder='Nhập từ khóa cần tìm kiếm' value={ this.state.svthKeyword } onChange={ e => this.setState({ svthKeyword: e.target.value }, this.fetchSinhVien) } />
                                </div>
                    ...
                    { this.state.showPopup.selectGVHD && <div className='popup-wrapper'>
                        <div className='popup'>
                            ...
                            <div className='popup__body'>
                                <div className='form-group'>
                                    <input type='text' className='form-input-disabled' placeholder='Nhập từ khóa cần tìm kiếm' value={ this.state.gvhdKeyword } onChange={ e => this.setState({ gvhdKeyword: e.target.value }, this.fetchGiangVien) } />
                                </div>
                    ...
                    { this.state.showPopup.selectGVPB && <div className='popup-wrapper'>
                        <div className='popup'>
                            ...
                            <div className='popup__body'>
                                <div className='form-group'>
                                    <input type='text' className='form-input-disabled' placeholder='Nhập từ khóa cần tìm kiếm' value={ this.state.gvpbKeyword } onChange={ e => this.setState({ gvpbKeyword: e.target.value }, this.fetchGiangVien) } />
                                </div>
                    ...
                    { this.state.showPopup.selectHDBV && <div className='popup-wrapper'>
                        <div className='popup'>
                            ...
                            <div className='popup__body'>
                                <div className='form-group'>
                                    <input type='text' className='form-input-disabled' placeholder='Nhập từ khóa cần tìm kiếm' value={ this.state.hdbvKeyword } onChange={ e => this.setState({ hdbvKeyword: e.target.value }, this.fetchGiangVien) } />
                                </div>
