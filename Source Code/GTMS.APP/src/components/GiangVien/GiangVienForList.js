...
import GiangVienForPrint from './GiangVienForPrint'
...
class GiangVienForList extends Component {
    constructor (props) {
      super(props)
      this.state = {
          data: [],
    ...
    async createMultiple() {
        const url = apiRoutes.giangVien.createMultiple
    ...
    async onTemporarilyDelete(id) {
        try {
            const url = apiRoutes.giangVien.temporarilyDeleteById + '/' + id
    ...
    async onPermanentlyDelete(id) {
        try {
            const url = apiRoutes.giangVien.permanentlyDeleteById + '/' + id
    ...
    async onRestore(id) {
        try {
            const url = apiRoutes.giangVien.restoreById + '/' + id
            const data = { trangThai: 1 }
    ...
    changeConfirmationValue(e) {
        ...
        const acceptedValue = this.state.activeRecord.maGiangVien.toString()
            ...
            case 'CURRENT_PAGE': {
                ...
                dataToExport = dataToExport.map(record => {
                    return {
                        ...
                        hocVi: record.HocVi || '',
                        maKhoa: record.khoa && record.khoa.maKhoa || '',
                        donViCongTac: record.donViCongTac,
    ...
    exportDocumentAsXlsx() {
        ...
        worksheet.getCell('A4').value = 'DANH SÁCH CÁC GIẢNG VIÊN HIỆN CÓ TRONG HỆ THỐNG'
        ...
        worksheet.getCell('B7').value = 'MSCB'
        ...
        worksheet.getCell('K7').value = 'Học hàm'
        ...
        worksheet.getCell('M7').value = 'Học vị'
        ...
        worksheet.getCell('O7').value = 'Khoa'
        ...
        worksheet.getCell('R7').value = 'Đơn vị công tác'
        ...
        this.state.dataToExportDocument.forEach((record, index) => {
            ...
            worksheet.getCell('B' + currentRowIndex).value = record.maGiangVien
            ...
            worksheet.getCell('K' + currentRowIndex).value = record.hocHam
            ...
            worksheet.getCell('M' + currentRowIndex).value = record.hocVi
            ...
            worksheet.getCell('O' + currentRowIndex).value = record.khoa && record.khoa.tenKhoa
            ...
            worksheet.getCell('R' + currentRowIndex).value = record.donViCongTac
        ...
        return (
            <Fragment>
                <section className='breadcrumbs'>
                    ...
                    <span className='breadcrumb-active'><Link to='/giang-vien'>Quản lý giảng viên</Link></span>
                </section>

                <section className='section'>
                    <header className='section__header'>
                        <div className='section__header-left'>
                            <h3 className='section__title'>Danh sách giảng viên</h3>
                            <p className='section__subtitle'>Danh sách tất cả các giảng viên hiện đang được quản lý</p>
                        </div>
                        ...
                        <div className='section-body-main'>
                            ...
                            <table className='table'>
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th onClick={ () => this.sortByColumn('MaGiangVien') }>{ this.isBeingSorted('MaGiangVien') && <i className='fas fa-sort sort-button'></i> } Mã giảng viên</th>
                                        ...
                                        <th onClick={ () => this.sortByColumn('HocHam') }>{ this.isBeingSorted('HocHam') && <i className='fas fa-sort sort-button'></i> } Học hàm</th>
                                        <th onClick={ () => this.sortByColumn('HocVi') }>{ this.isBeingSorted('HocVi') && <i className='fas fa-sort sort-button'></i> } Học vị</th>
                                        <th onClick={ () => this.sortByColumn('MaKhoa') }>{ this.isBeingSorted('MaKhoa') && <i className='fas fa-sort sort-button'></i> } Khoa</th>
                                        <th onClick={ () => this.sortByColumn('DonViCongTac') }>{ this.isBeingSorted('DonViCongTac') && <i className='fas fa-sort sort-button'></i> } Đơn vị công tác</th>
                                            ...
                                            <tr key={ index }>
                                                <td className='table-dropdown-menu-wrapper'>
                                                    ...
                                                    <ul className='table-dropdown-menu'>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/giang-vien/xem-thong-tin/' + record.maGiangVien }>Xem thông tin</Link></li>
                                                        <li className='table-dropdown-menu-item'><Link to={ '/giang-vien/cap-nhat/' + record.maGiangVien }>Cập nhật thông tin</Link></li>
                ...
                { this.state.showPopup.temporarilyDelete && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn xóa tạm thời giảng viên <strong>{ this.state.activeRecord.tenGiangVien }</strong> khỏi hệ thống hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onTemporarilyDelete(this.state.activeRecord.maGiangVien) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                    ...
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <Fragment>
                                <p className='alert-message'>Bạn có chắc chắn muốn xóa vĩnh viễn giảng viên <strong>{ this.state.activeRecord.tenGiangVien }</strong> khỏi hệ thống hay không?</p>
                                <p>Hành động này đồng nghĩa với việc dữ liệu sẽ bị xóa vĩnh viễn khỏi hệ thống và không thể khôi phục lại được. Để xác nhận, vui lòng nhập <span className='pre'>{ this.state.activeRecord.maGiangVien }</span> vào khung bên dưới</p>
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <Fragment>
                                { this.state.validConfirmationValue ? <span className='popup-button-danger' onClick={ () => this.onPermanentlyDelete(this.state.activeRecord.maGiangVien) }>Đồng ý</span> : <span className='popup-button-default'>Đồng ý</span> }
                            </Fragment> }
                        </footer>
                    </div>
                </div> }
                ...
                { this.state.showPopup.restore && <div className='popup-wrapper'>
                    <div className='popup'>
                        ...
                        <div className='popup__body'>
                            { !this.state.showPopupErrorMessage && <p>Bạn có chắc chắn muốn khôi phục lại giảng viên <strong>{ this.state.activeRecord.tenGiangVien }</strong> đã bị xóa tạm thời hay không?</p> }
                        ...
                        <footer className='popup__footer'>
                            ...
                            { !this.state.showPopupErrorMessage && <span className='popup-button-danger' onClick={ () => this.onRestore(this.state.activeRecord.maGiangVien) }>Đồng ý</span> }
                        </footer>
                    </div>
                </div> }
                ...
                <GiangVienForPrint data={ this.state.dataToExportDocument } />
            </Fragment>
        )
    }
}
...
export default connect(mapStateToProps, null)(GiangVienForList)